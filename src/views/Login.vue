<template>
  <div class="wrapper">
    ...
    box-shadow: gray 4px 4px 10px">
        ...
        <div style="margin: 20px 0;text-align: center;font-size: 24px;color: #153b7f"><b>SHU 后勤管理平台</b></div>
  ...
  methods:{
    ...
    login(){
      ...
      this.$refs["userForm"].validate((valid) => {
        if (valid) {
          //alert('登陆中!');
          this.request.post("/user/login?username="+this.user.username+"&password="+this.user.password).then(res=>{
            console.log("res:"+res);
            if(res.code==='200'){
