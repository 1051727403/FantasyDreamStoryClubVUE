<template>
  <div>
    ...
    <el-table :data="tableData"
      ...
      <el-table-column prop="shown" label="轮播图"  width="120">
        <template v-slot="scope" >
          <p v-if="tableData[scope.$index].shown">是</p>
          <p v-else>否</p>
        </template>
      </el-table-column>
...
export default {
  ...
  methods :{
    //load
    load(){
      this.request.get("/admin/activityPage",{
        params: {
        ...
        this.request.get('admin/activityNum').then(res=>{
          console.log(res);
          this.count = res.data;
        })
    ...
    saveAnnounce(){
      this.$refs['ruleForm'].validate((valid)=>{
        if(valid){
          ...
          this.request.post("/admin/upActivityInfo", {
            "id":this.form.id,
